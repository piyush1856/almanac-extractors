...
from extractors.serializers import SourceItemKind
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Failed to fetch project visibility: {response.status_code} - {response.text}")
                    return False
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Failed to fetch repository visibility: {response.status_code} - {response.text}")
                    return False
        except Exception as e:
...
class QuipExtractor(DataExtractor):
    ...
    async def _process_folder_recursively(self, folder_id, folder_name, folder_mapping, thread_ids, max_total_docs, source_url):
        ...
        if not folder_data:
            try:
                async with self.semaphore:
                    ...
                    response = await self.client.get(folder_url, headers=self.headers)
                    if response.status_code != 200:
                        logger.warning(f"Failed to fetch folder contents: {response.status_code}, {response.text}")
                        return
                    folder_data = response.json()
        ...
        logger.info(f"Found {len(thread_children)} threads and {len(folder_children)} subfolders")
        ...
        for child in thread_children:
            ...
            logger.info(f"Adding document '{thread_title}' from folder {folder_name} ({len(thread_ids)}/{max_total_docs})")
